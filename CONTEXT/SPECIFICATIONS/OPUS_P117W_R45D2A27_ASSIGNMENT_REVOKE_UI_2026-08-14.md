...
- rendre chaque attribution existante avec une action SCORE `Révoquer` ;
...
- une attribution déjà absente doit produire un message métier localisé ;
...
En admin/developer, la vue Attributions affiche `Révoquer` sur chaque attribution lorsque le backend le supporte. Une Preview de révocation non administrative montre `Accès perdus`. Une tentative de révocation de la dernière attribution administrative est refusée avant écriture avec un message explicite. En viewer, aucune action de mutation n’est visible.
