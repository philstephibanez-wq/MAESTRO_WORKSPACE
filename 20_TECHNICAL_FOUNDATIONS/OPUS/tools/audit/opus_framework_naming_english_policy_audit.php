...
/**
 * P112Q2A â€” Opus Framework Naming and English Policy Audit.
 *
 * This is an audit-only tool.
 *
 * It scans the current Opus framework and reports:
...
$outDir = $reportRoot . DIRECTORY_SEPARATOR . 'opus_naming_english_policy_' . $stamp;
...
$frenchTokenMap = [
    ...
    'libell[eÃ©]' => ['label', 'FRENCH_TOKEN'],
    'mot[_-]?de[_-]?passe' => ['password', 'FRENCH_TOKEN'],
    'param[eÃ¨]tre' => ['parameter_or_setting', 'FRENCH_TOKEN_REVIEW_CONTEXT'],
    'profil' => ['profile', 'FRANGLAIS_TOKEN'],
    'requ[eÃª]te' => ['request_or_query', 'FRENCH_TOKEN_REVIEW_CONTEXT'],
    ...
    'deconnexion|dÃ©connexion' => ['logout_or_disconnection', 'FRENCH_TOKEN_REVIEW_CONTEXT'],
...
csvWrite(
    $outDir . DIRECTORY_SEPARATOR . 'opus_directory_case_policy_audit.csv',
    ['relative_path', 'current_segment', 'proposed_segment', 'classification', 'reason'],
    $directoryFindings
);

csvWrite(
    $outDir . DIRECTORY_SEPARATOR . 'opus_english_policy_audit.csv',
    ['relative_path', 'line', 'token', 'suggestion', 'classification', 'excerpt'],
    $languageFindings
);

file_put_contents(
    $outDir . DIRECTORY_SEPARATOR . 'opus_naming_english_policy_summary.json',
    json_encode([
        'summary' => $summary,
...
$md = [];
$md[] = '# P112Q2A â€” Opus Naming and English Policy Audit';
$md[] = '';
...
$md[] = '## Directory policy';
$md[] = '';
$md[] = '`framework/Opus/<NamespaceSegment>/<ClassName>.php`';
$md[] = '';
...
file_put_contents($outDir . DIRECTORY_SEPARATOR . 'opus_naming_english_policy_audit.md', implode(PHP_EOL, $md) . PHP_EOL);
