...
/**
 * P112Q2A2 â€” Exhaustive Opus framework naming audit.
 *
 * This is audit-only.
 *
 ...
 * directory segment under framework/Opus and compares it with the proposed
...
$outDir = $reportRoot . DIRECTORY_SEPARATOR . 'opus_exhaustive_naming_audit_' . $stamp;
...
csvWrite(
    $outDir . DIRECTORY_SEPARATOR . 'opus_exhaustive_directory_naming_audit.csv',
    ['relative_path', 'current_segment', 'proposed_segment', 'classification', 'php_namespaces', 'namespace_segments'],
    $findings
);

csvWrite(
    $outDir . DIRECTORY_SEPARATOR . 'opus_exhaustive_namespace_segments_audit.csv',
    ['relative_path', 'current_segment', 'proposed_segment', 'classification', 'namespace', 'namespace_segment'],
    $namespaceFindings
);

file_put_contents(
    $outDir . DIRECTORY_SEPARATOR . 'opus_exhaustive_naming_summary.json',
    json_encode([
        'summary' => $summary,
...
$md = [];
$md[] = '# P112Q2A2 â€” Exhaustive Opus Naming Audit';
$md[] = '';
...
file_put_contents($outDir . DIRECTORY_SEPARATOR . 'opus_exhaustive_naming_audit.md', implode(PHP_EOL, $md) . PHP_EOL);
