...
final class LstsarBackgroundLifecycleScenario implements RecipeInterface, RobotScenario
{
    ...
    public function steps(): array
    {
        return [new RobotStep('schedule_and_run_background_job', function (RecipeContext $context, RobotSession $session): void {
            ...
            $context->assert($scheduled['status'] === \Opus\Lstsa\LstsaRunStatus::PENDING, 'OPUS_LIFE_LSTSAR_SCHEDULER_NOT_PENDING');
            $finished = (new \Opus\Lstsa\LstsaRunner($store))->runOnce('life_lstsar_background_runner');
            $context->assert(is_array($finished) && $finished['status'] === \Opus\Lstsa\LstsaRunStatus::DONE, 'OPUS_LIFE_LSTSAR_RUNNER_NOT_DONE');
