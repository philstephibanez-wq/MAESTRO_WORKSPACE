...
/**
 * P112Q3E2 delivery recipe.
 *
 * Public CLI delivery gate.
 * Role:
 *   Validate the ACL RefBook metadata feature, then run the global Opus
 *   anti-regression recipe to detect side effects before the delivery is accepted.
 */
$root = dirname(__DIR__, 2);
$reportRoot = $root . DIRECTORY_SEPARATOR . 'var' . DIRECTORY_SEPARATOR . 'reports' . DIRECTORY_SEPARATOR . 'p112q3e2_delivery_recipe';
...
$summary = [
    'palier' => 'P112Q3E2_DELIVERY_RECIPE',
...
$base = $reportRoot . DIRECTORY_SEPARATOR . 'P112Q3E2_DELIVERY_RECIPE_' . $timestamp;
...
if ($failed) {
    fwrite(STDERR, 'P112Q3E2_DELIVERY_RECIPE_FAILED' . PHP_EOL);
    exit(1);
}

echo 'P112Q3E2_DELIVERY_RECIPE_OK' . PHP_EOL;
exit(0);
...
function ensureDirectory(string $path): void
{
    ...
    if (!mkdir($path, 0775, true) && !is_dir($path)) {
        fwrite(STDERR, 'P112Q3E2_DELIVERY_RECIPE_DIRECTORY_FAILED: ' . $path . PHP_EOL);
        exit(1);
    }
}
...
function writeJson(string $path, array $data): void
{
    ...
    if (!is_string($json)) {
        fwrite(STDERR, 'P112Q3E2_DELIVERY_RECIPE_JSON_FAILED: ' . $path . PHP_EOL);
        exit(1);
    }
...
function writeText(string $path, string $content): void
{
    if (file_put_contents($path, $content) === false) {
        fwrite(STDERR, 'P112Q3E2_DELIVERY_RECIPE_WRITE_FAILED: ' . $path . PHP_EOL);
        exit(1);
    }
}
...
{
    return '<!doctype html><html lang="fr"><head><meta charset="utf-8"><title>P112Q3E2 Delivery Recipe</title></head><body><pre>'
