...
final class RoutingRecipe implements RecipeInterface
{
    ...
    public function run(RecipeContext $context): array
    {
        ...
        $router = \Opus\Routing\Router::fromXml($routes);
        $home = $router->match(new \Opus\Http\Request('/demo', 'GET'), $site);
        $context->assert($home->name === 'home', 'OPUS_ROUTING_HOME_MATCH_FAILED');
        $page = $router->match(new \Opus\Http\Request('/demo/about', 'GET'), $site);
        ...
        } catch (\Opus\Contract\ContractException) {
        }
