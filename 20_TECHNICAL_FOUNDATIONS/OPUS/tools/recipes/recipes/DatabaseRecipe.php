...
final class DatabaseRecipe implements RecipeInterface
{
    ...
    public function run(RecipeContext $context): array
    {
        ...
        $config = (new \Opus\Database\DatabaseMultiConfigLoader())->fromXml($xml, 'P112Q2J_DATABASE_RECIPE');
        $context->assert($config->defaultName() === 'source', 'OPUS_DATABASE_DEFAULT_CONNECTION_INVALID');
        $source = $config->get('source');
        $target = $config->get('target');
        $context->assert($source->normalizedProvider() === \Opus\Database\DatabaseProvider::SQLITE, 'OPUS_DATABASE_SOURCE_PROVIDER_INVALID');
        $context->assert($target->normalizedProvider() === \Opus\Database\DatabaseProvider::SQLITE, 'OPUS_DATABASE_TARGET_PROVIDER_INVALID');
